...
class Dashboard extends Component {
  ...
  clickViewEvent = (clickEvent, identifier, viewedEvent) => {
    this.modalRef.current.setState(
      { modalType: "ViewEvent", event: viewedEvent },
      () => {
        this.modalRef.current.toggleModal();
      }
    );
  ...
  clickEditEvent = (clickEvent, identifier, viewedEvent) => {
    this.modalRef.current.setState(
      { modalType: "EditEvent", event: viewedEvent },
      () => {
        this.modalRef.current.toggleModal();
      }
    );
  ...
  componentDidMount() {
    ...
    this.setState({rawUserInfo: userInfo});
    ...
    if (viewParam) {
      ...
      if (viewParam === "myevents") {
        ...
        API.getHouseholdEvents(
          userInfo.currentHouseholdId,
          userInfo.memberId,
          type
        )
          .then(res => {
  ...
  showHideChange = (toggleAction, eventDate) => {
    ...
    const newShowHideClass =
      toggleAction === "show" ? "show-event" : "hide-event";
    ...
    newEvents[dateIndex].events.map((event, i) => {
      ...
      if (i > 2) {
        ...
        const itemIndex = newEvents[dateIndex].events.findIndex(
          dateEvent => dateEvent.event_id === event.event_id
        );
    ...
    if (newEvents[dateIndex].events[itemIndex].assigned) {
      ...
      if (this.state.viewType === "assigned") {
        ...
        if (newEvents[dateIndex].events.length === 0) {
          ...
          newEvents[dateIndex].events.length >= 3 &&
  ...
  modifyEventStatus = (eventId, eventDate) => {
    ...
    newEvents[dateIndex].events[itemIndex].status =
      newEvents[dateIndex].events[itemIndex].status === "closed"
        ? "open"
        : "closed";
    // console.log(`event: ${newEvents[dateIndex].events[itemIndex].title}
    //              event id: ${newEvents[dateIndex].events[itemIndex].event_id}
  ...
  // Sets the state to change the view based upon the supplied viewOption
  clicker = viewParam => {

    let userInfo = this.state.rawUserInfo;
  ...
  render() {
    return (
      <section>
        <SideNav ref={this.sidenavRef} clicker={this.clicker} />
          ...
          <TopNav
            ...
            householdName={
              this.state.viewType === "all"
                ? `${this.state.householdName} - All Events`
                : this.state.viewType === "myevents"
                ? `My Events - ${this.state.firstName}`
                : this.state.viewType === "assigned"
                ? `Events Assigned to ${this.state.firstName}`
              ...
              <Col size="md-12 fluid">
                ...
                this.state.events.length > 0 ? (
                        ...
                        {eventDate.events.map((event, i) => {
                          ...
                          return (
                            <EventLine
                              ...
                              assigned={
                                event.assigned ? event.assigned : undefined
                              }
                  ...
                  <div className="no-events-msg">
                    <h3>Welcome back 😄 </h3>
                    <p>
                      It looks like there aren't any events for this house! To
                      add a new event, click the '+' sign below.
                    </p>
                  </div>
                )}
                <div>
        ...
        <ModalCardBody
          getHouseholdIdFunction={this.getHouseholdId}
          ref={this.modalRef}
        />
      </section>
    );
